# I followed one of the tutorials give by Kaggle to do this first attempt at the 
...
trainData["Child"] # creating grouping of children
for (i in 1:nrow(trainData)) {
...
testData["Child"] = NA # creating different categories
...
testData["Family"] = NA # making sure 
...
train.glm <- glm(Survived ~ Pclass + Sex + Age + Child + Sex*Pclass + Family + Mother, family = binomial, data = trainData) #the glm step
